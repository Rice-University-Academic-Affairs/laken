...
class TestLocalTables:
    def test_write_read_dbo(self, lakehouse, sample_df, df_kind):
        lakehouse.write_table(sample_df, "dbo.products")
    ...
    def test_bare_write_raises(self, lakehouse, sample_df):
    ...
    def test_overwrite_resets(self, lakehouse, sample_pandas):
        lakehouse.write_table(sample_pandas, "dbo.products")
        replacement = pd.DataFrame({"id": [99], "value": ["z"]})
        lakehouse.write_table(replacement, "dbo.products", mode="overwrite")
    ...
    def test_append_accumulates(self, lakehouse, sample_pandas):
        ...
        lakehouse.write_table(extra, "dbo.products", mode="append")
        result = lakehouse.read_table("products", as_="pandas")
        assert len(result) == 3

    def test_append_creates_missing_table(self, lakehouse, sample_pandas):
        lakehouse.write_table(sample_pandas, "dbo.products", mode="append")
        result = lakehouse.read_table("products", as_="pandas")
        assert len(result) == 2
